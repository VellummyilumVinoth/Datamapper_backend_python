...
async def create_record(file: UploadFile):
    ...
    try:
        if file.content_type == "application/pdf":
            ...
            file_content = record_extraction_with_gpt4(text_content=file_content, content_type=file.content_type)
            print(file_content)
        ...
        if file.content_type in ["image/jpeg", "image/png"]:
            ...
            with open(file_location, "rb") as image_file:
                ...
                img_summary = summarize_record_content_with_gpt4(base64_content=img_base64, content_type=file.content_type)
                file_content = record_extraction_with_gpt4(text_content=img_summary)
                print(file_content)
        ...
        if image_index < len(image_keys):
            ...
            if vobj['/Subtype'] == '/Image' and '/Filter' in vobj:
                ...
                if img:
                    ...
                    img_summary = summarize_record_content_with_gpt4(base64_content=img_base64,
...
def record_extraction_with_gpt4(text_content: str = None, base64_content: str = None, content_type: str = None) -> str:
    try:
...
@retry(stop=stop_after_attempt(5), wait=wait_exponential(multiplier=0.5, min=1, max=5), reraise=True)
def summarize_record_content_with_gpt4(base64_content: str = None, content_type: str = None) -> str:
    try:
